 ...
 */
final class Zinn_Cache_Style_Presets {
	...
	public static function fields( string $id ): array {
		...
		$fields = array(
			array(
				...
				'type'        => 'radio',
				'label'       => __( 'Style preset', 'zinn-cache' ),
				'description' => __( 'A starting point. Anything below overrides it.', 'zinn-cache' ),
				'choices'     => $choices,
			...
			array(
				...
				'type'        => 'toggle',
				'label'       => __( 'Use my own colours and sizes', 'zinn-cache' ),
				'description' => __( 'Leave this off and the preset is used exactly as it is.', 'zinn-cache' ),
				'default'     => false,
			),
		);
	...
	public static function resolve( string $id ): array {
		...
		$chosen  = (string) Zinn_Cache_Admin_UI::get( self::preset_key( $id ), (string) array_key_first( $presets ) );
		...
		if ( ! empty( Zinn_Cache_Admin_UI::get( self::custom_key( $id ), false ) ) ) {
			foreach ( (array) $component['tokens'] as $token => $spec ) {
				$value = (string) Zinn_Cache_Admin_UI::get( self::token_key( $id, (string) $token ), '' );
				if ( '' !== $value ) {
