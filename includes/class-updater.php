...
final class Updater {
	 ...
	 */
	private const CACHE_KEY = 'zinn_cache_update_payload';
	 ...
	 * Constructor.
	 *
	 * @param string $file    Main plugin file (`ZINN_CACHE_FILE`).
	 * @param string $version Installed version (`ZINN_CACHE_VERSION`).
	 */
	...
	public function plugin_info( $result, string $action, $args ) {
		...
		return (object) array(
			...
			'requires_php'  => $release['requires_php'],
			'sections'      => array(
				'changelog' => '' !== $release['changelog'] ? $release['changelog'] : esc_html__( 'See the Zinn Digital® release notes.', 'zinn-cache' ),
			),
		);
	}
	...
	public function verify_download( $reply, $package, $upgrader = null, $hook_extra = array() ) {
		...
		if ( ! is_string( $actual ) || ! hash_equals( strtolower( $release['package_sha256'] ), strtolower( $actual ) ) ) {
			...
			return new WP_Error(
				...
				__( 'The downloaded update failed its integrity check and was not installed.', 'zinn-cache' )
			);
		}
